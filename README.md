...
*上图展示了SuperMro分析示例项目examples/sample_project 生成的继承关系图：ImageProcessor（图像处理器）和TextProcessor（文本处理器）都是可以实际使用的类，它们都继承自DataProcessor（通用处理器）；DataProcessor继承自BaseProcessor（处理器模板）。图中每个类下方列出了该类的主要公共方法，帮助开发者快速了解项目中的类的功能接口。*
